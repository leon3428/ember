...
class EventBus {
 public:
  template <typename T, typename Fn>
    requires std::is_pointer_v<decltype(&T::idn)>
  void subscribe(Fn &&fn) {
    if (!m_callbacks.contains(T::idn)) {
      m_callbacks[T::idn] = {new m_CallbackList<T>{}, [](void *p) { delete static_cast<m_CallbackList<T> *>(p); }};
    }
  ...
  void fire(const T &&data) const {
    ...
    if (m_callbacks.contains(T::idn)) p = static_cast<m_CallbackList<T> const *>(m_callbacks.at(T::idn).get());

    if (p) {
      for (const auto &callback : *p) callback(data);
    }
  }
 ...
 private:
  ...
  std::unordered_map<Identifier, std::unique_ptr<void, std::function<void(void *)>>, IdentifierHash> m_callbacks;
};
}  // namespace ember
