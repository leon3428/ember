...
auto ember::static_mesh::load(Identifier idn, const ResourceMap &resourceMap) -> StaticMesh {
  auto path = resourceMap.getPath(idn);

  if (path) {
    Assimp::Importer importer;

    auto scene =
        importer.ReadFile(*path, aiProcess_CalcTangentSpace | aiProcess_Triangulate | aiProcess_JoinIdenticalVertices |
  ...
  throw std::runtime_error("Identifier does not exist in resource map");
}